...
exports.exec = function (table, indiceName, region, es_domain, es_endpoint, callback) {
		...
		let promise = when.promise(function(resolve, reject) {
			...
			es.indices.exists({ //check whether the indices exists or not
				...
				if (status == 200) { //if indices found
					...
					createIndex(es, indiceName, function() { //create indices
						resolve({es: es, domain: result.domain});
					});
				}
				else {
			...
			if (!err) {
				var recordsList = _.map(records.Items, function(record) { //for every record found
					return when.promise(function(resolve, reject) {
						recordExists(result.es, indiceName, record).then(function(exists) {//check if record exists or not
							return putRecord(result.es, indiceName, record, exists);
...
const createIndex = function(es, indiceName, callback) {
	...
	recordExists = function(es, indiceName, record) { // check if record exists in elasticsearch
		return when.promise(function(resolve, reject) {
			es.get({
				index: indiceName,
				id: record.sortKey, //this is the primaryKey of record in elasticsearch from which we can check weather the record is exists or not
				type: '_all'
			}, function(err, response, status) {
	...
	putRecord = function(es, indiceName, record, exists) { //insert record in elasticsearch
		return when.promise(function(resolve, reject) {

			var params = {
				index: indiceName,
				id: record.sortKey, //this will be primaryKey of record in elasticsearch
				body: record,
				type: 'datatype'
			};
